...
let ECH_DNS = 'udp://208.67.220.220:443';
...
async function _getECH() {
  ...
  try {
    ...
    const res = await fetch(/^https:\/\//i.test(ECH_DNS) ? ECH_DNS : "https://dns.alidns.com/dns-query", {
      method: 'POST',
...
async function pCL(text, uuid, h) {
  try {
    ...
    if (ECH) {
      // Clash/Mihomo 通过 query-server-name 动态查询 ECHConfig
    ...
    while(i<L.length){const l=L[i],tl=l.trim();
      if(tl.startsWith('- {')&&tl.includes('uuid:')){
        ...
        if(um&&um[1].trim()===uuid.trim()){
          ...
          if(ECH) fn=fn.replace(/\}(\s*)$/, ', '+_eo+': {enable: true, '+_qsn+': '+ECH_SNI+'}}$1');
        }
        R.push(fn);i++;
        ...
        if(um&&um[1].trim()===uuid.trim()){
          ...
          if(ECH&&ii>=0){const ind=' '.repeat(bi+2);const echLines=[ind+_eo+':',ind+'  enable: true',ind+'  '+_qsn+': '+ECH_SNI];nl.splice(ii+1,0,...echLines);}}
        R.push(...nl);
      }else{R.push(l);i++;}}
    return R.join('\n');
  } catch (e) { return text; }
...
function dashPage(host, uuid, proxyip, subpass, subdomain, converter, subToken, env, clientIP, hasAuth, tgState, cfState, add, addApi, addCsv, tgToken, tgId, cfId, cfToken, cfMail, cfKey, sysParams, dashTitle, proxyCheckUrl, dls, echEnabled, echSni, echDns, statsEnabled, statsChatId, zoneId) {
    ...
    const longLink = subToken
        ? `https://${subdomain}/sub?base=${encodeURIComponent(baseNode)}&token=${encodeURIComponent(subToken)}`
        : regularLongLink;
    ...
    <script>
        ...
        function updateLink() {
            ...
            if (subToken) {
                ...
                const desireSearch = new URLSearchParams();
